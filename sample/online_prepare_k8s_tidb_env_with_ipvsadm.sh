...
EOF
  ...
  echo -e "\033[32m*\033[0m"
  echo "prepare ipvs"
  ${ssh_cmd} ${sshUser}@${host} "cat > /etc/sysconfig/modules/ipvs.modules" <<EOF
#!/bin/bash
modprobe -- ip_vs
modprobe -- ip_vs_rr
modprobe -- ip_vs_wrr
...
EOF
  ...
  ${ssh_cmd} ${sshUser}@${host} "chmod 755 /etc/sysconfig/modules/ipvs.modules && bash /etc/sysconfig/modules/ipvs.modules && lsmod | grep -e ip_vs -e nf_conntrack_ipv4"
